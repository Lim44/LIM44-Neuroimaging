...
try
    ...
    for b=1:length(params.blocks)
        ...
        if isequal(params.blocks{b},'experiment')
            time_out = 2; % Define it based on training later            
        ...
        while params.time_block >= timer_block % Should think of better names for these two variables
            ...
            % Select ITI for the current trial
            ITI = 1;                        
            
            % Execute one trial
            [response(count_operation(col_exp), col_exp),time,log] = executeMISTtrial(operation,time_out,pctg_correct_flag,params);
